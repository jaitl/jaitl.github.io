...
Под это определение подходит `multi-master` конфигурация, в которой каждая нода может независимо обрабатывать запросы на запись и чтение и при потере связи, репликация останосится, но ноды продолжат работать независимо, хоть и не будут знать об новых данных друг у друга.
...
Но это очень странная конфигурация, не думаю что кто-то будет такое делать в реальной прадакшен системе.

## Типы баз данных
Рассмотрим популярные типы баз данных с точки зрения CAP теоремы.

### Реляционные базы данных (Relational Database, RDBM)
### Ключ-значение (Key-value database)
...
Эта БД поддерживает только `multi-master` репликацию данных. 
